    ...
    PrimitiveBlock: Block + 'static,
{
    ...
    async fn subscribe_blocks(&self, tx: Sender<Self::Block>) {
        ...
        let mut backoffs: Vec<Duration> = vec![Duration::from_secs(1); total];

        loop {
            for (i, url) in self.urls.iter().enumerate() {
                if backoffs[i] > Duration::from_secs(1) {
                ...
                let provider =
                    ...
                    .await
                    {
                        ...
                        Err(err) => {
                            ...
                            backoffs[i] = (backoffs[i] * 2).min(Duration::from_secs(30));
                            continue;
                        }
                    };
                ...
                loop {
                    match tokio::time::timeout(self.block_timeout, inner_rx.recv()).await {
                        Ok(Some(block)) => {
                            backoffs.fill(Duration::from_secs(1));
